...
import { getBsigEntityClassLabel, getEntityClassFieldLabel, getJurisdictionLabel, shouldShowEntityClass } from './regulatory';
...
function getRegulatorySectionTitle(regulatoryProfile: RegulatoryProfile): string {
  return `${getJurisdictionLabel(regulatoryProfile.jurisdiction)}-Regime`;
}

function buildRegulatoryMetaRows(regulatoryProfile: RegulatoryProfile): Array<[string, string]> {
  if (shouldShowEntityClass(regulatoryProfile)) {
    return [[getEntityClassFieldLabel(regulatoryProfile), getBsigEntityClassLabel(regulatoryProfile.bsigEntityClass, regulatoryProfile)]];
  }
  return [['Jurisdiktion', getJurisdictionLabel(regulatoryProfile.jurisdiction)]];
}

function buildRegimeSummaryLines(regimeSummaries: RegulatoryRegimeSummary[]): string[] {
  return regimeSummaries.map((summary) => {
    const scopeLabel = summary.scopeStatus === 'in_scope'
      ? 'im Scope'
      : summary.scopeStatus === 'out_of_scope'
        ? 'außer Scope'
        : 'Einordnung offen';
    return `${summary.shortLabel}: ${scopeLabel}, ${summary.openRequirements} offene Bausteine, ${summary.checklistBlockers} Blocker`;
  });
}
...
export function exportEvidenceRegisterAsCsv(evidenceItems: EvidenceItem[]): void {
  const rows = [
    ...
    ...evidenceItems.map((item) => [
      ...
      item.link,
      item.tags.join(' | '),
      item.serverAttachment?.fileName ?? item.attachment?.fileName ?? '',
      item.sourceLabel,
      item.notes,
    ]),
  ];
...
${buildRegimeSummaryLines(regimeSummaries).map((line) => `- ${line}`).join('\n')}
...
<body>
  ...
  <h2>${htmlEscape(getRegulatorySectionTitle(regulatoryProfile))}</h2>
  <div class="card">
    ${buildRegulatoryMetaRows(regulatoryProfile).map(([label, value]) => `<div class="row"><span>${htmlEscape(label)}</span><strong>${htmlEscape(value)}</strong></div>`).join('')}
    ${regimeSummaries.map((summary) => `<div class="row"><span>${htmlEscape(summary.label)}</span><strong>${summary.scopeStatus === 'out_of_scope' ? 'außer Scope' : `${summary.openRequirements} offen / ${summary.checklistBlockers} Blocker`}</strong></div>`).join('')}
  </div>
...
export function exportManagementReportAsPdf(params: {
  ...
  doc.addSection(getRegulatorySectionTitle(regulatoryProfile));
  doc.addMiniTable([
    ...buildRegulatoryMetaRows(regulatoryProfile),
    ...regimeSummaries.map((summary) => ([summary.shortLabel, summary.scopeStatus === 'out_of_scope' ? 'außer Scope' : `${summary.openRequirements} offen / ${summary.checklistBlockers} Blocker`] as [string, string])),
  ]);
  ...
  doc.addMiniTable([
    ['Anhänge hinterlegt', documentLibrarySummary.attachedFiles],
