...
function validateQuestion(
  ...
  if (typeof question.domainId === 'string' && !validDomainIds.has(question.domainId)) {
    errors.push(`${path}.domainId "${question.domainId}" ist im Paket 1 nicht unterstützt.`);
  }

  return errors.length === 0;
}

function validateRequirement(requirement: unknown, errors: string[], path: string): boolean {
...
export function validateModuleObject(input: unknown): ModuleValidationResult {
  ...
  if (input.kritisExtension !== undefined) {
    if (!isRecord(input.kritisExtension)) {
      ...
      if (extension.eligibleSectors !== undefined && !Array.isArray(extension.eligibleSectors)) {
        errors.push('kritisExtension.eligibleSectors muss ein Array sein.');
      }
      if (extension.hints !== undefined && !Array.isArray(extension.hints)) {
        errors.push('kritisExtension.hints muss ein Array sein.');
      }
