...
function withDownloadSession(url: string, token?: string): string {
  if (!url || !token) {
    return url;
  }
        ...
        {evidence.serverAttachment ? (
          <div className="attachment-pill">
            ...
            <div className="inline-actions">
              <a
                className="button tertiary"
                href={withDownloadSession(evidence.serverAttachment.url, downloadToken)}
                download={evidence.serverAttachment.fileName}
              >
                <Download size={16} />
                Download
              </a>
          ...
          {versionEntries?.length ? versionEntries.map((entry) => (
            <div key={entry.id} className="timeline-row">
              ...
              <div className="inline-actions">
                ...
                <a className="button tertiary" href={withDownloadSession(entry.downloadUrl, downloadToken)} download={entry.fileName}>
                  <Download size={16} />
                  Datei
                </a>
