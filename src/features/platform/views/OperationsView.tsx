...
function statusTone(status: 'ok' | 'warn' | 'missing' | undefined): 'success' | 'warn' | 'danger' {
  if (status === 'ok') {
    return 'success';
  }
  if (status === 'warn') {
...
function readinessTone(status: HostingReadinessSummary['status'] | SecurityGateSummary['status'] | undefined): 'good' | 'warn' | 'alert' | 'default' {
  if (status === 'ready') {
    return 'good';
  }
...
function readinessLabel(status: HostingReadinessSummary['status'] | SecurityGateSummary['status'] | undefined): string {
  if (status === 'ready') {
    return 'Betriebsreif';
  }
  if (status === 'progressing') {
...
function restoreTone(status: RestoreDrillSummary['overallStatus'] | undefined): 'good' | 'warn' | 'alert' | 'default' {
  if (status === 'passed') {
    return 'good';
  }
  if (status === 'warning') {
...
export function OperationsView({
  ...
  const restoreDrillJobs = jobRuns.filter((job) => job.type === 'restore_drill');
    ...
    <div className="view-stack">
      <section className="hero card">
        <div>
          ...
          <div className="chip-row top-gap">
            ...
            <span className="chip outline">Observability: {systemSettings.observabilityMode}</span>
            <span className="chip outline">WAF-lite: {systemSettings.wafLiteEnabled ? 'aktiv' : 'deaktiviert'}</span>
      ...
      <section className="stats-grid phase-three">
        ...
        <StatCard
          ...
          value={`${securityGateSummary?.overallScore ?? 0}%`}
          subtitle={`${securityGateSummary?.blockers ?? 0} Blocker · ${securityGateSummary?.warnings ?? 0} Warnungen`}
          tone={readinessTone(securityGateSummary?.status)}
        />
        <StatCard
          title="Observability"
          value={`${observabilitySummary?.totalRequests ?? 0}`}
          subtitle={`p95 ${observabilitySummary?.p95LatencyMs ?? 0} ms · Fehlerquote ${observabilitySummary?.errorRatePercent ?? 0}%`}
          tone={(observabilitySummary?.errorRatePercent ?? 0) > 10 ? 'warn' : 'good'}
        />
        <StatCard
          title="Restore-Drills"
          value={`${restoreDrills.length}`}
          subtitle={latestRestoreDrill ? `${latestRestoreDrill.overallStatus} · ${formatDateTime(latestRestoreDrill.createdAt)}` : 'Noch kein Drill'}
          tone={restoreTone(latestRestoreDrill?.overallStatus)}
        />
        <StatCard
          title="Integrität"
          value={`${integritySummary?.issueCount ?? 0}`}
          subtitle={integritySummary ? `${integritySummary.filesChecked} Dateien geprüft` : 'Noch kein Scan'}
          tone={(integritySummary?.highCount ?? 0) > 0 ? 'alert' : (integritySummary?.issueCount ?? 0) > 0 ? 'warn' : 'good'}
        />
        <StatCard
          title="Mandanten"
          value={`${availableTenants.length}`}
          subtitle={`${productionTenantCount} in Produktion · ${activeClientCount} aktive API-Clients`}
      ...
      {!hasSystemAdminAccess ? (
        <div className="feedback-box warn">
          ...
          <p className="top-gap">
            Der normale Arbeitsbereich bleibt ohne Anmeldung nutzbar. Für Systemprofil, API-Clients,
            Restore-Drills, Betriebsparameter und Mandantenpflege ist jedoch eine aktive
            Systemadministrationssitzung erforderlich.
          </p>
        </div>
      ) : null}
      ...
      <section className="content-grid two-column">
          ...
          <div className="mini-list top-gap">
            {(readinessSummary?.checks || []).map((check) => (
              <div key={check.id} className="timeline-row">
                ...
                <span className={`chip ${statusTone(check.status)}`}>{check.status}</span>
              </div>
            ))}
        ...
        <article className="card">
          <div className="section-heading">
            <div>
              <p className="eyebrow">Security Gates</p>
              <h3>Härtung, Secrets und Freigaben</h3>
            </div>
            <div className="inline-note">
              <ShieldEllipsis size={16} />
              <span>{readinessLabel(securityGateSummary?.status)}</span>
            </div>
          </div>
          <div className="mini-list top-gap">
            {(securityGateSummary?.gates || []).map((gate) => (
              <div key={gate.id} className="timeline-row">
                <div>
                  <strong>{gate.label}</strong>
                  <p className="muted small top-gap">{gate.detail}</p>
                </div>
                <span className={`chip ${statusTone(gate.status)}`}>{gate.status}</span>
              </div>
            ))}
            {!securityGateSummary?.gates.length ? <p className="muted">Noch keine Security-Gates verfügbar.</p> : null}
          </div>
        </article>
      </section>
      ...
      <section className="content-grid two-column">
        <article className="card">
          <div className="section-heading">
            <div>
              ...
              <h3>Request-Telemetrie und letzte Sicherheitsereignisse</h3>
            </div>
            <div className="inline-note">
              <Activity size={16} />
              <span>{observabilitySummary ? `${observabilitySummary.totalRequests} Requests` : 'Noch keine Daten'}</span>
            </div>
          </div>
          <div className="stats-grid top-gap compact-grid">
            <StatCard title="Aktive Requests" value={`${observabilitySummary?.activeRequests ?? 0}`} subtitle="Momentan offen" tone="default" />
            <StatCard title="Fehlerquote" value={`${observabilitySummary?.errorRatePercent ?? 0}%`} subtitle="HTTP ≥ 400" tone={(observabilitySummary?.errorRatePercent ?? 0) > 10 ? 'warn' : 'good'} />
            <StatCard title="p95 Latenz" value={`${observabilitySummary?.p95LatencyMs ?? 0} ms`} subtitle="über alle Routen" tone={(observabilitySummary?.p95LatencyMs ?? 0) > 400 ? 'warn' : 'default'} />
            <StatCard title="Letzte Aktivität" value={formatDateTime(observabilitySummary?.lastRequestAt || '')} subtitle="Request-Telemetrie" tone="default" />
          </div>
          <div className="work-list top-gap">
            {(observabilitySummary?.routes || []).slice(0, 6).map((route) => (
              <article key={route.route} className="work-card">
                <div className="work-card-head">
                  <div>
                    <strong>{route.route}</strong>
                    <p className="muted small top-gap">{route.count} Requests · Fehler {route.errorCount} · zuletzt {formatDateTime(route.lastSeenAt)}</p>
                  </div>
                  <Radar size={18} />
                </div>
                <div className="chip-row">
                  <span className="chip outline">Ø {route.averageLatencyMs} ms</span>
                  <span className="chip outline">p95 {route.p95LatencyMs} ms</span>
                  <span className="chip outline">Status {route.lastStatus}</span>
                </div>
              </article>
            ))}
          </div>
          <div className="mini-list top-gap">
            {(observabilitySummary?.recentEvents || []).slice(0, 8).map((event) => (
              <div key={event.id} className="timeline-row">
                <div>
                  <strong>{event.kind === 'security' ? 'Security' : 'Request'} · {event.route}</strong>
                  <p className="muted small top-gap">{event.detail} · ID {event.requestId || '–'}</p>
                </div>
                <span className={`chip ${event.kind === 'security' ? 'warn' : 'outline'}`}>{event.status}</span>
              </div>
            ))}
            {!observabilitySummary?.recentEvents.length ? <p className="muted">Noch keine Telemetriedaten vorhanden.</p> : null}
          </div>
        </article>
        ...
        <article className="card">
          <div className="section-heading">
            <div>
              <p className="eyebrow">Restore-Drills</p>
              <h3>Wiederherstellungsfähigkeit regelmäßig prüfen</h3>
            </div>
            <div className="inline-note">
              <HardDriveDownload size={16} />
              <span>{latestRestoreDrill ? latestRestoreDrill.overallStatus : 'Kein Drill'}</span>
            </div>
          </div>
          <div className="inline-actions top-gap">
            <button type="button" className="button secondary" onClick={() => onRunSystemJob({ type: 'restore_drill' })} disabled={!hasSystemAdminAccess}>
              <RotateCcw size={16} />
          ...
          <div className="work-list top-gap">
            {restoreDrills.map((drill) => (
              <article key={drill.jobId} className="work-card">
                <div className="work-card-head">
                  <div>
                    <div className="question-title-row">
                      <strong>{drill.tenantScope}</strong>
                      <span className={`chip ${drill.overallStatus === 'passed' ? 'success' : drill.overallStatus === 'warning' ? 'warn' : 'danger'}`}>{drill.overallStatus}</span>
                    </div>
                    <p className="muted small top-gap">{formatDateTime(drill.createdAt)} · ausgelöst von {drill.triggeredBy}</p>
                  </div>
                </div>
                <div className="chip-row">
                  <span className="chip outline">{drill.tenantCount} Mandanten</span>
                  <span className="chip outline">Backups ok {drill.verifiedBackups}</span>
                  <span className="chip outline">fehlend {drill.missingBackups}</span>
                  <span className="chip outline">Snapshots fehlend {drill.missingSnapshots}</span>
                </div>
                {drill.recommendations.length ? (
                  <div className="mini-list top-gap">
                    {drill.recommendations.slice(0, 3).map((recommendation) => (
                      <div key={recommendation} className="mini-list-row">
                        <span>Hinweis</span>
                ...
                {drill.downloadUrl ? (
                  <div className="inline-actions">
                    <button
                      ...
                      onClick={() => onDownloadJobArtifact({
                        id: drill.jobId,
                        type: 'restore_drill',
                        label: 'Restore-Drill',
                        tenantId: '',
                        tenantName: drill.tenantScope,
                        status: 'done',
                        startedAt: drill.createdAt,
                        completedAt: drill.createdAt,
                        triggeredBy: drill.triggeredBy,
                        summary: 'Restore-Drill Artefakt',
                        artifactFileName: drill.artifactFileName,
                        downloadUrl: drill.downloadUrl,
                      })}
                    >
                      <Download size={16} />
                      Artefakt laden
            ...
            {!restoreDrills.length ? <p className="muted">Noch keine Restore-Drills registriert.</p> : null}
          </div>
        </article>
      </section>
      ...
      <section className="content-grid two-column">
        <article className="card">
          <div className="section-heading">
            <div>
              ...
              <h3>Persistenz-, Sicherheits- und Observability-Parameter</h3>
            </div>
            <div className="inline-note">
              <CloudCog size={16} />
          ...
          <div className="form-grid two-column top-gap">
            ...
            <label className="field-label">
              ...
              <select
                ...
                disabled={!hasSystemAdminAccess}
              >
                <option value="sqlite-document-store">sqlite-document-store</option>
                <option value="supabase-rest-store">supabase-rest-store</option>
            ...
            <label className="field-label">
              Observability-Modus
              <select
                value={settingsDraft.observabilityMode}
                onChange={(event) => setSettingsDraft((current) => ({ ...current, observabilityMode: event.target.value as SystemSettings['observabilityMode'] }))}
                disabled={!hasSystemAdminAccess}
              >
            ...
            <label className="field-label">
              ...
              <input
                type="number"
                min={7}
                value={settingsDraft.logRetentionDays}
                onChange={(event) => setSettingsDraft((current) => ({ ...current, logRetentionDays: Number(event.target.value) || 7 }))}
                disabled={!hasSystemAdminAccess}
              />
            </label>
            <label className="field-label">
              Restore-Drill-Kadenz in Tagen
              <input
                type="number"
                min={1}
                value={settingsDraft.restoreDrillCadenceDays}
                onChange={(event) => setSettingsDraft((current) => ({ ...current, restoreDrillCadenceDays: Number(event.target.value) || 1 }))}
                disabled={!hasSystemAdminAccess}
              />
            </label>
            <label className="field-label">
              Security-Review-Kadenz in Tagen
              <input
                type="number"
                min={7}
                value={settingsDraft.securityReviewCadenceDays}
                onChange={(event) => setSettingsDraft((current) => ({ ...current, securityReviewCadenceDays: Number(event.target.value) || 7 }))}
                disabled={!hasSystemAdminAccess}
              />
            </label>
            <label className="field-label wide">
          ...
          <div className="chip-row top-gap">
            ...
            <button
              type="button"
              className={`status-toggle ${settingsDraft.wafLiteEnabled ? 'selected' : ''}`}
              onClick={() => hasSystemAdminAccess && setSettingsDraft((current) => ({ ...current, wafLiteEnabled: !current.wafLiteEnabled }))}
              disabled={!hasSystemAdminAccess}
            >
              WAF-lite
            </button>
          ...
          <div className="inline-actions top-gap">
            <button type="button" className="button secondary" onClick={() => setSettingsDraft(systemSettings)} disabled={!hasSystemAdminAccess}>
              <RotateCcw size={16} />
              Entwurf zurücksetzen
            </button>
            <button type="button" className="button primary" onClick={() => onUpdateSystemSettings(settingsDraft)} disabled={!hasSystemAdminAccess}>
              <DatabaseZap size={16} />
        ...
        <article className="card">
          ...
          <div className="form-grid two-column top-gap">
            <label className="field-label wide">
              Bezeichnung
              <input value={clientLabel} onChange={(event) => setClientLabel(event.target.value)} disabled={!hasSystemAdminAccess} />
            </label>
            <label className="field-label">
              Mandant
            ...
            <label className="field-label wide">
              Notiz
              <input value={clientNote} onChange={(event) => setClientNote(event.target.value)} disabled={!hasSystemAdminAccess} />
            </label>
          </div>
          <div className="chip-row top-gap">
        ...
        <article className="card">
          <div className="section-heading">
            <div>
              ...
              <h3>Backups, Integritätsscans, Exportinventar und Restore-Drills</h3>
            </div>
            <div className="inline-note">
              <Download size={16} />
              ...
              return (
                <article key={tenant.id} className="work-card">
                  ...
                  <div className="form-grid two-column">
                    ...
                    <label className="field-label">
                      Datenregion
                      <input value={String(draft.dataRegion || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], dataRegion: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                    <label className="field-label">
                      Primärer Ansprechpartner
                      <input value={String(draft.primaryContactName || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], primaryContactName: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                    <label className="field-label">
                      Primäre E-Mail
                      <input type="email" value={String(draft.primaryContactEmail || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], primaryContactEmail: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                    <label className="field-label">
                      Technischer Ansprechpartner
                      <input value={String(draft.technicalContactName || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], technicalContactName: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                    <label className="field-label wide">
                      Technische E-Mail
                      <input type="email" value={String(draft.technicalContactEmail || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], technicalContactEmail: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                    <label className="field-label wide">
                      Notiz
                      <textarea value={String(draft.notes || '')} onChange={(event) => setTenantDrafts((current) => ({ ...current, [tenant.id]: { ...current[tenant.id], notes: event.target.value } }))} disabled={!hasSystemAdminAccess} />
                    </label>
                  </div>
                  <div className="inline-actions">
                    <button type="button" className="button primary" onClick={() => onUpdateTenant(tenant.id, draft)} disabled={!hasSystemAdminAccess}>
                      <Building2 size={16} />
                      Mandant speichern
                    </button>
                  </div>
      ...
      <section className="content-grid two-column">
        <article className="card">
          <div className="section-heading">
            <div>
              ...
              <h3>Vorbereitete API-Endpunkte und Probes</h3>
            </div>
            <div className="inline-note">
              <Server size={16} />
              <span>{serverHealth?.features.length ?? 0} Serverfeatures</span>
            </div>
          </div>
          <div className="work-list top-gap">
            <article className="work-card"><div className="work-card-head"><div><strong>/api/integration/manifest</strong><p className="muted small">Systemmanifest, Features und Basisparameter</p></div><Server size={18} /></div></article>
            <article className="work-card"><div className="work-card-head"><div><strong>/api/health/live</strong><p className="muted small">Liveness-Probe für Container und Uptime-Checks</p></div><CheckCircle2 size={18} /></div></article>
            <article className="work-card"><div className="work-card-head"><div><strong>/api/health/ready</strong><p className="muted small">Readiness-Probe für Persistenz und Betriebsstatus</p></div><ShieldCheck size={18} /></div></article>
          </div>
        </article>
        ...
        <article className="card">
          <div className="section-heading">
            <div>
              <p className="eyebrow">Restore-Jobhistorie</p>
              <h3>Letzte Drill-Läufe im Jobregister</h3>
            </div>
            <div className="inline-note">
              <Fingerprint size={16} />
              <span>{restoreDrillJobs.length} Drill-Jobs</span>
            </div>
          </div>
          <div className="mini-list top-gap">
            {restoreDrillJobs.slice(0, 8).map((job) => (
              <div key={job.id} className="timeline-row">
                <div>
                  <strong>{job.label}</strong>
                  <p className="muted small top-gap">{job.summary}</p>
                </div>
                <span className="chip outline">{formatDateTime(job.completedAt || job.startedAt)}</span>
              </div>
            ))}
            {!restoreDrillJobs.length ? <p className="muted">Noch keine Restore-Drill-Jobläufe vorhanden.</p> : null}
          </div>
        </article>
      </section>
    </div>
