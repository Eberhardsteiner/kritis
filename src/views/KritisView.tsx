    ...
    <div className="view-stack">
      <section className="content-grid two-column">
        ...
        <article className="card">
          ...
          <h3>Audit- und Readiness-Steuerung</h3>
          <p>{gateStatus.text}</p>
      ...
      <section className="card">
        <div className="section-heading">
          <div>
            ...
            <h3>Revisionssichere Dossiers und Freigabe</h3>
          </div>
          <div className="chip-row">
            <span className="chip outline">{dossiers.length} registrierte Dossiers</span>
        ...
        <div className="form-grid two-column top-gap">
          ...
          <label className="field-label">
            Zeichnende Stelle / Rolle
            <input
              type="text"
              placeholder={certificationAuthorityLabel || 'Interne Prüfstelle'}
          ...
          <label className="field-label wide">
            ...
            <textarea
              rows={3}
              placeholder="Scope, Restrisiken, offene Punkte, geplanter Review"
              value={dossierNote}
        ...
        <div className="inline-actions top-gap">
          <button
            ...
            onClick={() => {
              onCreateCertificationDossier('certification_dossier', {
                title: dossierTitle,
                note: dossierNote,
                signOffName: dossierSignOffName,
                signOffRole: certificationAuthorityLabel || 'Interne Prüfstelle',
              });
              setDossierTitle('');
              setDossierNote('');
            }}
            ...
            <article key={entry.id} className="priority-item">
              <div>
                ...
                <p className="muted small">
                  ...
                  {' · '}
                  {entry.userName || 'ohne Nutzer'}
                  {entry.signOffRole ? ` · ${entry.signOffRole}` : ''}
                </p>
                <p className="muted small">Checksumme: {entry.checksumSha256 ? `${entry.checksumSha256.slice(0, 14)}…` : '–'}</p>
                {entry.note ? <p className="muted small top-gap">{entry.note}</p> : null}
                {entry.releaseNote ? <p className="muted small top-gap">Freigabenotiz: {entry.releaseNote}</p> : null}
              </div>
                ...
                {entry.releaseStatus !== 'released' ? (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => onReleaseExportPackage(entry.id, releaseNoteMap[entry.id] ?? '')}
                    >
                      <ShieldCheck size={16} />
                      Freigeben
                    </button>
      ...
      <section className="card">
        <div className="section-heading">
          <div>
            ...
            <h3>Prüfkatalog für internes Audit und Readiness-Entscheidung</h3>
          </div>
          <div className="inline-actions">
