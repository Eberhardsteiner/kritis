...
11. 🔄 App-Shell `src/app/`: in vier Sub-Iterationen aufgeteilt (C2.11a–d). Zielkorridor siehe Abschnitt „Ziel-Stand nach C" (450–650 Zeilen):
    ...
    - C2.11b: State-Hydration-Layer (`buildAppStateFromLoaded`, `applyRemoteState`, Normalizer) nach `src/app/state/`.
    - C2.11c: Server-Sync-Layer (`loadStateFromServer`, `refreshServerSideData`, 4 useEffects) nach `src/app/serverSync/`. Die fünf Server-Sync-Push-Loop-Invarianten aus C2.7c werden bei jeder Änderung explizit geprüft.
    - C2.11d: Context-Provider-Layer (`WorkspaceStateContext` + `AppDerivedStateContext`) und Feature-Hook-Dep-Reduktion.
